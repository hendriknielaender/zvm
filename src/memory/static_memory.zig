...
pub const StaticMemory = struct {
    ...
    pub fn calculate_memory_size() usize {
        ...
        assert(total > 0);
        assert(total < 1024 * 1024 * 1024); // Less than 1GB
    ...
    pub fn init(backing_buffer: []u8) StaticMemory {
        assert(backing_buffer.len > 0);
        assert(backing_buffer.len == calculate_memory_size());
        assert(@intFromPtr(backing_buffer.ptr) % 8 == 0); // 8-byte aligned
        ...
        assert(result.buffer.len == backing_buffer.len);
    ...
    pub fn get_usage(self: *const StaticMemory) MemoryUsage {
        assert(self.buffer.len > 0);
        assert(self.fba.end_index <= self.buffer.len);
        ...
        assert(usage.used + usage.available == usage.total);
    ...
    pub const MemoryUsage = struct {
        ...
        pub fn format(
            ...
            assert(self.available == self.total - self.used);
...
comptime {
    ...
    assert(required > 0);
    assert(required >= 1024 * 1024); // At least 1MB
    assert(mb > 0);
    assert(mb == required / (1024 * 1024));
