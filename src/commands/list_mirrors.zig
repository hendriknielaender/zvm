...
pub fn execute(
    ...
    } else if (emitter.config.mode == .plain) {
        // Plain: index\turl\tmaintainer per line, no header, no usage hint.
        // Why: usage hints are decoration that breaks `awk -F'\t'` parsing.
        var line_buffer: [512]u8 = undefined;
        for (config.zig_mirrors, 0..) |mirror_info, index| {
            std.debug.assert(index < 1024);
            const url = mirror_info[0];
            const maintainer = mirror_info[1];
            std.debug.assert(url.len > 0);
            std.debug.assert(maintainer.len > 0);
